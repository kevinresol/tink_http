...
/**
  Shared client + endpoint fixture for case-specific TestHttp suites.
  Public test methods live only on the case subclasses so SuiteAsm can
  register exactly the groups selected by `-D cases=`.
**/
@:timeout(20000)
class TestHttp {
  ...
  public function new(client:ClientType, target:Target) {
    ...
    switch this.target = target {
      case Httpbin(true):
        url = HttpbinConfig.secureUrl;
        converter = new HttpbinConverter();
      case Httpbin(false):
        url = HttpbinConfig.url;
        converter = new HttpbinConverter();
      case Local(port):
  ...
  function request(method:Method, url:Url, ?headers:Array<HeaderField>, ?body:IdealSource):Promise<EchoedRequest> {
    if (headers == null) headers = [];
    final header = new OutgoingRequestHeader(method, url, headers);
...
/** Case ids: `methods`, `query`, `body` (any triggers registration). */
@:timeout(20000)
@:asserts
class TestHttpMethods extends TestHttp {
  @:variant(GET)
  @:variant(POST)
  @:variant(PATCH)
  @:variant(DELETE)
...
/** Case ids: `headers`, `headers-multi` (any triggers registration). */
@:timeout(20000)
@:asserts
class TestHttpHeaders extends TestHttp {
...
@:timeout(20000)
@:asserts
class TestHttpOrigin extends TestHttp {
...
@:timeout(20000)
@:asserts
class TestHttpChunked extends TestHttp {
  public function chunked() {
    final streamUrl:Url = url + '/stream/10';
    final headers = [new HeaderField(HOST, streamUrl.host.toString())];
    client.request(new OutgoingRequest(
      ...
      return res.body.all().next(function(chunk) {
        final lines = [for (line in chunk.toString().split('\n')) if (line.length > 0) line];
